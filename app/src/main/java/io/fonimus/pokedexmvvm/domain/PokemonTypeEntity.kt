...
import io.fonimus.pokedexmvvm.catchOrDefault
import java.util.*
...
enum class PokemonTypeEntity(val color: Int, val icon: Int) {
    ...
    BUG(R.color.type_bug, R.drawable.ic_baseline_bug_report_24),
    ...
    companion object {
        fun parse(type: String?): PokemonTypeEntity? {
            if (type == null) {
                return null
            }
            return catchOrDefault(NORMAL) { valueOf(type.uppercase(Locale.getDefault())) }
        }
    }
